...
c_beta <- 0.001#(2.38^2)/(P + length(intercept))
c_eps <- 0.001 # (2.38^2)
c_nu <- 0.001 # (2.38^2)/(3 + 1)
c_rho <- rep(0.001, 3) #rep((2.38^2)/(3 + 1), 3)
...
system.time(
  out_metcpp <- tgmrf(data = bd, formula = formula,
                      ...
                      nu = nu, rho_s = rho_s, rho_t = rho_t, rho_st = rho_st,
